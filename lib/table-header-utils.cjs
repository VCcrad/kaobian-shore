/**
 * 双行/多行错位表头识别（CJS，供 table-job-parser 使用）
 */
...
function inferSubHeaderStartColumn(dataRow, subHeaders, normalizeEducation, mainHeaderCount = 0) {
...
function buildExtendedHeaders(mainHeaders, subHeaders, subStartCol) {
...
/** 将多行表头标签写入 extended（如「岗位」+「名称」→ 岗位名称） */
...
function resolveCompositeHeaderLayout(rows, headerIdx, normalizeEducation) {
