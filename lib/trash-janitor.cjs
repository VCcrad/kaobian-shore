...
async function runTrashJanitor(prisma) {
  ...
  const evaporatedJobPostings = await prisma.jobPosting.deleteMany({
    where: {
      isDeleted: true,
      deletedAt: { lt: sevenDaysAgo },
    },
  ...
  return {
    ...
    jobPostingsArchived: archiveResult.jobPostings ?? 0,
    evaporatedJobs: evaporatedJobs.count,
    evaporatedJobPostings: evaporatedJobPostings.count,
